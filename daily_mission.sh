...
if [ -f .env ]; then
    echo "正在加载 .env 配置..." >> run.log
    # source命令会读取文件并执行里面的 export 语句
    source .env
else
    echo "错误: 找不到 .env 文件！任务终止。" >> run.log
    exit 1
fi
...
"$PYTHON_EXEC" main.py \
    --base-url https://open.bigmodel.cn/api/paas/v4 --model "autoglm-phone" --apikey "$GLM_KEY" \
    "[执行步骤]
    ...
    步骤5：最后，请把手机返回到桌面。
    [提示]
    1.如何判断已经签到：如果有「对号」的卡片的**下一个卡片**右上方没有红点，说明今天已经完成签到了
    2.禁止补签，签到日期不一定和卡片上的时间对应
    " \
    >> run.log 2>&1
